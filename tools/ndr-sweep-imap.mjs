...
// consulted. That is ground truth about our own infrastructure - stronger than a
// self-probe (public resolvers refuse Spamhaus queries) and it is what actually stops
// mail. Patterns are deliberately narrow: a bare "[1.2.3.4]" is usually the RECEIVER's
// own host, so only phrasings that unambiguously refer to the connecting client count.
