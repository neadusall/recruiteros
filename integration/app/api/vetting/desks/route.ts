...
export async function POST(req: Request) {
  ...
  switch (b.action) {
    case "provision": {
      ...
      // A desk-level voice pick wins; otherwise the workspace's default ElevenLabs
      // voice (VOICE_CLONE_VOICE_ID, the house "Lukas" voice) satisfies the gate,
      // resolved isolation-correctly so it's lent to granted white-label tenants
      // too. provisionDesk already falls back to the same default at synth time.
      const hasVoice =
