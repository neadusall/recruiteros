/**
 * Post Recruiter: find qualified people who are open to work, and reach them.
 *
...
import {
  postRecruiterView, scanWorkspace, saveHunt, removeHunt, toggleHunt,
...
export async function GET(req: Request): Promise<Response> {
  ...
  const view = await postRecruiterView(g.ctx.workspace.id);
...
export async function POST(req: Request): Promise<Response> {
  ...
  const view = () => postRecruiterView(ws);
