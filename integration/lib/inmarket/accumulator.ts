...
async function runShotTick(): Promise<void> {
  ...
  try {
    ...
    // Drive from CONTACTABLE curated prospects: they carry the resolved company DOMAIN (the pool leads
    // don't), which is what the capture needs to find + verify the company's own careers page. We have
    // no per-role posting URL to hand it, so each capture does careers-page discovery — a strict,
    // lower-yield path, so coverage builds gradually; the ones that pass are real company-site shots.
    const list = await listCurated({ contactableOnly: true, limit: 5000 }).catch(() => [] as Array<{ company?: string; role?: string; domain?: string }>);
    const seen = new Set<string>();
    const todo: Array<{ company: string; role: string; domain?: string }> = [];
    for (const p of list) {
      if (!p.company || !p.role || !p.domain) continue;    // need a domain to have any chance at the gate
