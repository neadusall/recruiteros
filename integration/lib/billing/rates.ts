...
export const DEFAULT_RATES: CostRate[] = [
  ...
  },
  {
    ...
    note: "PLACEHOLDER rate, separate from landline by design. Wire a cheap RapidAPI mobile-lookup listing into RAPIDAPI_MOBILE_HOST/PATH, then set this to its real blended cost. Mobile is the hardest/lowest-yield field on the cheap tier (~10-20% usable), so budget ~2x per usable number. OFF by default.",
    scales: "per_prospect",
  },
  {
    id: "landline_find",
  ...
  },
  {
    ...
    label: "Mobile, premium reveal (backup, on miss only)",
    category: "enrichment",
    unitCostUsd: 0.25,
    unit: "per mobile resolved",
    note: "Premium mobile reveal (PDL / Apollo / ContactOut / Lusha class), $0.10-0.78 each; mobiles cost more than direct dials. ONLY reached when the cheap mobile lookup misses. NOT in the base estimate, shown so the fallback tier is auditable.",
    scales: "per_prospect",
  },
  {
    ...
    unitCostUsd: 0.1,
    unit: "per landline resolved",
    note: "Premium direct-dial reveal, typically cheaper than mobile. ONLY reached when the cheap landline lookup misses. NOT in the base estimate.",
    scales: "per_prospect",
  },
