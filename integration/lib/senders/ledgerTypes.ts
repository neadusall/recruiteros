...
export const CAUSES: CauseDef[] = [
  ...
  },
  {
    ...
    title: "Bounces with no sends behind them",
    meaning: "This mailbox has recorded bounces but still no recorded sends AFTER counter reconciliation ran, so its bounce rate cannot be computed and the health guard's bounce rule cannot fire on it. Something sent from this mailbox through a path that writes to no send log we read. This is a hole in the measurement, not a verdict on the mailbox, and a hole is worse than a bad number because it reads as a clean bill.",
    provenBy: "The senders registry after reconciliation against the per-box tally published by the deliverability tool. Before that tally existed this state was universal on the cold lane; now it means a genuinely unlogged send path.",
    fix: "Find what sent from this address. Every lane that transmits must land in a send log the reconciliation reads, or call recordSend() directly. Do not read the blank rate as a good rate until it does.",
    wearPerDay: 0,
  },
  {
    code: "counters.mismatched",
    category: "capacity", severity: "info", blocking: false,
    title: "Delivery notices exceed campaign sends",
    meaning: "More bounce notices have landed in this mailbox than it has campaign sends, which is arithmetically impossible for one population and therefore proof they are two. The excess is warm-up traffic bouncing. It is worth seeing, because warm-up mail failing at this volume is itself a signal, but it must never be divided by campaign sends: that is what produced a 10,400% bounce rate before the matched pair existed.",
    provenBy: "The registry's notices-seen counter read against the matched campaign pair published by the deliverability tool.",
    fix: "Nothing for the mailbox. If the warm-up bounce volume is high, look at the warm-up lane and the sending IP, not at this mailbox's campaign performance.",
    wearPerDay: 0,
  },
  {
    code: "ledger.stale",
