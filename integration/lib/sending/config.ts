...
interface SendingConfigState {
  hetznerDnsToken?: string; // encrypted at rest
  hcloudToken?: string;     // encrypted at rest
  smartleadApiKey?: string; // encrypted at rest
...
export async function setSendingConfig(patch: SendingConfigPatch): Promise<void> {
  await hydrate();
  const setSecret = (k: "hetznerDnsToken" | "hcloudToken" | "smartleadApiKey", v: unknown) => {
...
function stored(k: "hetznerDnsToken" | "hcloudToken" | "smartleadApiKey"): string | undefined {
  const v = state[k];
...
export interface SendingConfigStatus {
  ...
  source: { dns: "portal" | "env" | "none"; cloud: "portal" | "env" | "none"; smartlead: "portal" | "env" | "none" };
}
