...
const CATALOG: IntegrationMeta[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "SERPER_API_KEY", label: "Wide pass · Serper API key (top-up)", required: false, secret: true, placeholder: "paste your serper.dev key", hint: "Optional second wide pass, behind DataForSEO. Serper.dev serves real Google results at roughly $0.30-$1 per 1,000 searches (2,500 free credits to start), no daily cap. It runs AFTER DataForSEO and only on queries DataForSEO did not fill, so it is a top-up rather than the main engine. Its credit bundle can expire without warning, which is why it is no longer the primary. Get a key at serper.dev → API Key." },
      ...
      { key: "DATAFORSEO_LOGIN", label: "Primary pass · DataForSEO login", required: false, placeholder: "the email you signed up with", hint: "THE MAIN WIDE-WEB ENGINE — set this one first. DataForSEO returns up to 100 Google results in a single request (measured on this account: about $0.02 a query, ~76 LinkedIn profiles back), and it runs BEFORE Serper so it absorbs the volume first. It is pay-as-you-go with a balance that cannot silently expire the way a Serper credit bundle can. Sign up at app.dataforseo.com → API Access." },
      { key: "DATAFORSEO_PASSWORD", label: "Primary pass · DataForSEO password", required: false, secret: true, placeholder: "paste your API password", hint: "The API password from app.dataforseo.com → API Access. This is NOT your dashboard login password." },
      { key: "DATAFORSEO_MAX_QUERIES", label: "Primary pass · max searches per run", required: false, placeholder: "100", hint: "Optional spend guard: how many of a run's queries may use DataForSEO. Default 100, 300 on a Wide net search, and DOUBLED automatically when Serper is unavailable so the primary pass can cover the whole run on its own. Setting a value here pins it and turns that automatic top-up off. Budget roughly $0.02 per query." },
    ],
    steps: [
      ...
      "PRIMARY PASS (do this one) → add a DataForSEO login + API password from app.dataforseo.com → API Access. This is the main wide-web engine: up to 100 Google results per request, about $0.02 a query on this account, and a pay-as-you-go balance that cannot expire out from under a run. It goes first and absorbs the volume.",
      "TOP-UP PASS (optional) → add a serper.dev API key: real Google results at roughly $0.30-$1 per 1,000 searches, no daily cap. It runs AFTER DataForSEO, on the queries DataForSEO did not fill. Having both means a drained balance on one can never stop a search.",
