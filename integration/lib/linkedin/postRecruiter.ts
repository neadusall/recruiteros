...
const KEY = "linkedin_post_recruiter_v1";

/** Saved searches worked per tick. Each costs one LinkedIn search on a seat. */
const HUNTS_PER_TICK = Math.max(1, Number(process.env.POST_RECRUITER_HUNTS_PER_TICK ?? 2));
/** Search results pulled per hunt per tick. */
const SEARCH_PAGE_SIZE = Math.max(5, Number(process.env.POST_RECRUITER_PAGE_SIZE ?? 25));
/**
 ...
 *
 * Post Recruiter needs far fewer reads than Role Hunter did, because the search
 ...
 */
const READS_PER_SCAN = Math.max(1, Number(process.env.POST_RECRUITER_READS_PER_SCAN ?? 6));
const READ_GAP_MIN_MS = Math.max(0, Number(process.env.POST_RECRUITER_READ_GAP_MIN_MS ?? 2_000));
const READ_GAP_MAX_MS = Math.max(READ_GAP_MIN_MS, Number(process.env.POST_RECRUITER_READ_GAP_MAX_MS ?? 9_000));
...
const VIEWS_PER_SEAT_PER_DAY = Math.max(1, Number(process.env.POST_RECRUITER_VIEWS_PER_SEAT ?? 25));

/** Touches per seat per working day, before jitter. */
const TOUCHES_PER_SEAT = Math.max(1, Number(process.env.POST_RECRUITER_TOUCHES_PER_SEAT ?? 12));
const TOUCHES_PER_SEAT_WEEK = Math.max(1, Number(process.env.POST_RECRUITER_TOUCHES_PER_WEEK ?? 60));
/** Drafts auto-executed per tick when autopilot is armed. */
const AUTO_PER_TICK = Math.max(1, Number(process.env.POST_RECRUITER_AUTO_PER_TICK ?? 8));

/** Never engage a post older than this. Free to enforce: see activityDate(). */
const MAX_POST_AGE_DAYS = Math.max(1, Number(process.env.POST_RECRUITER_MAX_POST_AGE_DAYS ?? 30));
/** Indexed-post window. qdr:w is 100% fresh, qdr:m is 96% at ~1.5x the volume. */
const POST_TIME_WINDOW = process.env.POST_RECRUITER_TIME_WINDOW ?? "qdr:m";
/** Never re-touch the same person inside this window, desk-wide. */
const RETOUCH_DAYS = Math.max(1, Number(process.env.POST_RECRUITER_RETOUCH_DAYS ?? 30));
...
const MODEL = () => process.env.POST_RECRUITER_MODEL ?? "claude-haiku-4-5";
...
export type PrChannel = "comment" | "connect" | "message";
export type PrStatus = "draft" | "queued" | "sent" | "failed" | "skipped" | "replied";
...
interface PrState {
  ...
  dayStats: Record<string, Record<string, PrDayStats>>;
...
async function hydrate(): Promise<void> {
  if (!hydrated) {
    hydrated = loadSnapshot<PrState>(KEY)
      .then((snap) => {
...
function statsFor(workspaceId: string): PrDayStats {
 ...
 * the whole reason Post Recruiter can afford to run: the Recruiter search item
 ...
 */
function scoreFit(hunt: PrHunt, p: {
...
function postQueryFor(hunt: PrHunt): string {
...
function fillCopy(tpl: string, l: PrLead, hunt: PrHunt): string {
...
function draftFor(l: PrLead, hunt: PrHunt): string | null {
...
function seedHash(s: string): number {
  ...
  // last byte across all 32 bits. Caught by scripts/test-post-recruiter.mts.
...
function huntsFor(workspaceId: string): PrHunt[] {
...
function makeLead(
  workspaceId: string, hunt: PrHunt, seat: LiAccountState, hit: SearchHit,
...
async function runSearchLane(
  workspaceId: string, hunt: PrHunt, accounts: LiAccountState[], budget: { reads: number },
): Promise<number> {
...
export async function scanWorkspace(workspaceId: string, opts: { huntId?: string } = {}): Promise<{
  ...
  for (const hunt of take) {
    ...
    catch (e) { console.log(`[post-recruiter] search lane "${hunt.label}": ${e instanceof Error ? e.message : e}`); }
    if (budget.reads <= 0) break;
    try { created += await runPostLane(workspaceId, hunt, accounts, budget); }
    catch (e) { console.log(`[post-recruiter] post lane "${hunt.label}": ${e instanceof Error ? e.message : e}`); }
  }
  ...
  if (autopilotOn(workspaceId)) {
    ...
    if (sent) console.log(`[post-recruiter] ${workspaceId}: autopilot queued ${sent}`);
  }

  console.log(`[post-recruiter] ${workspaceId}: hunts=${take.length} created=${created} reads=${READS_PER_SCAN - budget.reads}`);
  return { created, reads: READS_PER_SCAN - budget.reads };
}

/** Every workspace with at least one active saved search. */
export async function tickPostRecruiter(): Promise<void> {
  await hydrate();
  const ids = [...new Set(state.hunts.filter((h) => h.active).map((h) => h.workspaceId))];
  for (const ws of ids) {
    try { await scanWorkspace(ws); }
    catch (e) { console.log(`[post-recruiter] ${ws} scan failed: ${e instanceof Error ? e.message : e}`); }
  }
}
...
export async function saveHunt(workspaceId: string, input: Dict, ownerEmail?: string): Promise<PrHunt> {
  await hydrate();
  const id = str(input.id);
  const existing = id ? state.hunts.find((h) => h.id === id && h.workspaceId === workspaceId) : undefined;
  const titles = splitList(input.titles);
  const hunt: PrHunt = existing ?? {
    id: rid("prh"),
    workspaceId,
    label: "",
    keywords: "",
...
export async function pushToCandidates(
  ...
  const name = (listName || "").trim() || "Post Recruiter";
  try {
    ...
    if (!campaignId) {
      campaignId = (await createCampaign({
        ...
        goal: "Candidates confirmed open to work by Post Recruiter",
...
export interface PrView {
  ...
  stats: Array<{ day: string } & PrDayStats>;
}

export async function postRecruiterView(workspaceId: string): Promise<PrView> {
  await hydrate();
  prune();
...
export async function postRecruiterHealth(workspaceId: string): Promise<{
...
export const __postRecruiterTestHooks = {
  activityDate,
  scoreFit,
  sellerReason,
  sellerPostReason,
  ...
  SEEKER_RE,
  SELLER_RE,
  reset(next?: Partial<PrState>) { state = { ...empty(), ...(next ?? {}) }; hydrated = Promise.resolve(); },
  state() { return state; },
  seatRoom,
  allowanceFor,
  pickSendSeat,
