...
(async () => {
  ...
  const f = pathToFileURL(path.resolve("public/video-preview-dark.html")).href;
  await page.goto(f, { waitUntil: "load", timeout: 30000 }).catch((e) => console.log("nav:", e.message));
  await page.waitForTimeout(5000);
  await page.screenshot({ path: ".preview/fold-dark.png" });   // viewport only — shows exactly the fold
  await browser.close();
