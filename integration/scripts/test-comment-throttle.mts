...
check("defaults are the documented ones", () => {
  ...
  assert.equal(l.perDay, 8);
  assert.equal(l.perWeek, 35);
});
...
check("the day allowance varies across days and stays in the jitter band", () => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    assert.ok(a >= 5 && a <= 11, `allowance ${a} outside the +/-40% band around 8`);
    seen.add(a);
  }
  assert.ok(seen.size >= 4, `only ${seen.size} distinct allowances in 30 days: not varied enough`);
});
...
check("the weekly ceiling is hard and outranks the day", () => {
  ...
  const now = Date.now();
  hooks.setLog(WS, Array.from({ length: 35 }, (_, i) =>
    new Date(now - (i + 1) * 4 * 3_600_000).toISOString()));
  const t = commentThrottleFor(WS);
  assert.equal(t.weekUsed, 35);
