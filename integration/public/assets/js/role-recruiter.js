 ...
 *
 * The SPA route "rolerecruiter" (renderRoleRecruiter in command.js) is a thin
 * controller over window.__RoleRecruiter; every action goes through
 * /api/role-recruiter and the shared LinkedIn engine behind it. Same shape as
...
(function () {
  ...
  var API = (window.RECRUITEROS_API_BASE || "") + "/api/role-recruiter";
  ...
  /* Ceilings mirror the server (roleRecruiter.ts). Shown live so a recruiter
  ...
  function render(el) {
    host = el;
    el.innerHTML = '<div class="rr-wrap"><div class="empty">Loading Role Recruiter...</div></div>';
    refresh();
  }
  ...
  function refresh() {
    return apiGet("").then(function (d) {
      ...
      if (host) host.innerHTML = '<div class="rr-wrap"><div class="empty">Role Recruiter could not load. Refresh the page.</div></div>';
    });
  }
  ...
  function paintBusy(on) {
    $$(".rr-wrap [data-busy]").forEach(function (b) { b.disabled = !!on; });
  }
  ...
  /* No <h1> here on purpose. The portal shell already prints the page title in
     the header (and hangs the Features Video pill beside it), so a second one
     inside the panel reads as a duplicate heading. This is the standfirst only. */
  function headerHtml() {
  ...
  function noticesHtml() {
    var out = "";
    if (view.lastError) {
      out += '<div class="rr-note bad"><span class="rr-note-ic">!</span><span><b>Discovery is blocked.</b> ' + esc(view.lastError) + "</span></div>";
    }
    if (view.reasons && view.reasons.length) {
      out += '<div class="rr-note warn"><span class="rr-note-ic">!</span><span><b>Not hunting yet.</b> ' +
    ...
    if (view.spotlight && !view.spotlight.ok) {
      out += '<div class="rr-note info"><span class="rr-note-ic">i</span><span>' +
      ...
      out += '<div class="rr-note"><span class="rr-note-ic">*</span><span>' +
  ...
  function barHtml() {
    ...
    return '<div class="rr-bar">' +
      '<span class="rr-state"><span class="rr-dot ' + cls + '"></span>' + label + "</span>" +
      '<span class="rr-num">Last scan <b>' + esc(ago(view.lastScan)) + "</b></span>" +
      '<span class="rr-bar-spacer"></span>' +
      '<label class="rr-switch" title="Send approved-quality drafts without waiting for a tap">' +
        '<input type="checkbox" id="rrAuto"' + (view.autopilot ? " checked" : "") + ' data-busy>' +
        '<span class="rr-track"></span> Autopilot' +
      "</label>" +
      '<span class="rr-num" title="Touches per seat, per working day">Per day ' +
        '<input class="rr-limit" id="rrPerDay" type="number" min="1" max="40" value="' + (lim.perDay | 0) + '" data-busy></span>' +
      '<span class="rr-num" title="Touches per seat, per rolling week">Per week ' +
        '<input class="rr-limit" id="rrPerWeek" type="number" min="1" max="200" value="' + (lim.perWeek | 0) + '" data-busy></span>' +
      '<button class="btn btn-ghost btn-sm" id="rrPause" data-busy>' + (view.paused ? "Resume" : "Pause") + "</button>" +
      '<button class="btn btn-sm" id="rrScan" data-busy>Scan now</button>' +
      "</div>";
  }
  ...
  function talliesHtml() {
    ...
    function cell(v, k, good) {
      return '<div class="rr-tally-cell' + (good && v ? " good" : "") + '">' +
        '<span class="rr-tally-v">' + (v | 0) + "</span><span class=\"rr-tally-k\">" + esc(k) + "</span></div>";
    }
    return '<div class="rr-tally">' +
  ...
  function tabsHtml() {
    ...
    return '<div class="rr-tabs" role="tablist">' + tabs.map(function (t) {
      return '<a class="rr-tab' + (t[0] === tab ? " active" : "") + '" href="#" role="tab" aria-selected="' +
  ...
  function huntsHtml() {
    ...
    if (!hunts.length && !editing) {
      return out + '<div class="empty">No saved searches yet. Add one and Role Recruiter starts hunting on the next scan.</div>';
    }
    out += '<div class="rr-hunts">' + hunts.map(function (h) {
      var bits = [];
      ...
      return '<div class="rr-hunt' + (h.active ? "" : " off") + '">' +
        '<div class="rr-hunt-main">' +
          '<span class="rr-hunt-name">' + esc(h.label) + "</span>" +
          '<span class="rr-hunt-sub">' + (bits.length ? bits.map(function (x) { return "<span>" + x + "</span>"; }).join("") : "<span>Everywhere in the US</span>") + "</span>" +
          '<span class="rr-hunt-stats">' +
          ...
          "</span>" +
        "</div>" +
        '<div class="rr-hunt-acts">' +
          '<label class="rr-switch" title="' + (h.active ? "Pause this search" : "Resume this search") + '">' +
            '<input type="checkbox" data-huntactive="' + esc(h.id) + '"' + (h.active ? " checked" : "") + ' data-busy>' +
            '<span class="rr-track"></span></label>' +
  ...
  function huntFormHtml(h) {
    ...
    function f(id, label, val, hint, attrs) {
      return '<div class="rr-field"><label for="' + id + '">' + esc(label) + "</label>" +
        '<input id="' + id + '" value="' + esc(val == null ? "" : val) + '" ' + (attrs || "") + ">" +
        (hint ? '<span class="hint">' + hint + "</span>" : "") + "</div>";
    }
    return '<div class="rr-form">' +
      '<div class="rr-grid">' +
        f("rrfLabel", "Name this search", h.label, "What you will recognise it by in this list.") +
        f("rrfTitles", "Job titles", (h.titles || []).join(", "),
          "Comma separated. These decide who counts as a fit, and the first one is what the post lane searches for.") +
        f("rrfCreds", "Must hold", (h.credentials || []).join(", "),
          "Licences or certifications, comma separated. Leave blank if the desk does not need one.") +
        f("rrfLocation", "Location", h.location, "Free text, for example Dallas, Texas.") +
        f("rrfRadius", "Radius (miles)", h.radiusMiles, "Optional. Needs a LinkedIn location id to apply as a hard filter.", 'type="number" min="0" max="500"') +
        f("rrfLocId", "LinkedIn location id", h.locationId, "Optional. Paste from a Recruiter search URL to filter on LinkedIn's side.") +
        f("rrfYears", "Minimum years", h.minYears, "Optional experience floor.", 'type="number" min="0" max="50"') +
        f("rrfPitch", "Role you are pitching", h.pitchRole, "Used in the outreach copy, for example Clinical Director.") +
        f("rrfKeywords", "Search keywords", h.keywords, "Boolean is allowed. Leave blank to search the job titles above.") +
      "</div>" +
      '<div class="rr-form-acts">' +
  ...
  function queueHtml() {
    var rows = (view.queue || []);
    if (!rows.length) {
      return '<div class="empty">Nothing waiting. Role Recruiter adds people here as it confirms them, ' +
        "roughly every twenty minutes while a search is active.</div>";
    }
    return '<div class="rr-leads">' + rows.map(leadHtml).join("") + "</div>";
  }
  ...
  function leadHtml(l) {
    ...
    var post = "";
    if (l.postText) {
      post = '<div class="rr-post">' + esc(l.postText.slice(0, 340)) + (l.postText.length > 340 ? "..." : "") +
    ...
    var statusChip = l.status === "queued" ? '<span class="rr-chip queued">Scheduled</span>'
      : l.status === "failed" ? '<span class="rr-chip failed">Not sent</span>' : "";

    return '<div class="rr-lead' + (l.status === "failed" ? " failed" : "") + '" data-lead="' + esc(l.id) + '">' +
      '<div class="rr-lead-top">' +
        '<div class="rr-lead-id">' +
          '<span class="rr-lead-name">' + name +
            '<span class="rr-chip otw">Open to work</span>' +
            '<span class="rr-chip ' + esc(l.channel) + '">' + esc(CHANNEL_LABEL[l.channel] || l.channel) + "</span>" +
            statusChip +
          "</span>" +
          '<span class="rr-lead-head">' + esc(l.headline || l.currentTitle || "") + "</span>" +
          '<span class="rr-lead-meta">' + meta.map(function (m) { return "<span>" + m + "</span>"; }).join("") + "</span>" +
        "</div>" +
        '<div class="rr-lead-right">' +
          '<div class="rr-fit"><span class="rr-fit-v ' + fitCls + '">' + (l.fit | 0) + '</span><span class="rr-fit-k">fit</span></div>' +
        "</div>" +
      "</div>" +
      '<div class="rr-why"><span>' + esc(l.evidence) + "</span>" +
        (l.fitWhy || []).map(function (w) { return "<span>" + esc(w) + "</span>"; }).join("") + "</div>" +
      post +
      (l.statusReason ? '<div class="rr-note bad" style="padding:8px 11px">' + esc(l.statusReason) + "</div>" : "") +
      (pending
        ? '<textarea class="rr-draft" data-draft="' + esc(l.id) + '" maxlength="' + (max + 60) + '">' + esc(l.draft || "") + "</textarea>"
        : '<div class="rr-post" style="border-left-color:var(--brand)">' + esc(l.draft || "") + "</div>") +
      '<div class="rr-lead-acts">' +
        (pending
        ...
        '<span class="rr-count' + (len > max ? " over" : "") + '" data-count="' + esc(l.id) + '">' + len + " / " + max + "</span>" +
      "</div>" +
    "</div>";
  }
    ...
    return '<div class="rr-table-wrap"><table class="rr-table"><thead><tr>' +
      ...
      "</tr></thead><tbody>" + rows.map(function (l) {
        return "<tr>" +
          ...
          '<td><span class="rr-chip ' + esc(l.channel) + '">' + esc(CHANNEL_LABEL[l.channel] || l.channel) + "</span></td>" +
  ...
  function activityHtml() {
    ...
    if (!seats.length) out += '<div class="empty">No LinkedIn seat is connected.</div>';
      ...
      out += '<div class="rr-table-wrap"><table class="rr-table"><thead><tr>' +
        ...
        "</tr></thead><tbody>" + seats.map(function (s) {
          ...
          return "<tr><td><b>" + esc(s.name || s.accountId) + "</b></td>" +
            ...
            '<td><span class="rr-meter"><i class="' + (tp >= 100 ? "full" : tp >= 75 ? "hot" : "") + '" style="width:' + tp + '%"></i></span></td>' +
            '<td class="n">' + (s.views | 0) + " / " + (s.viewCap | 0) + "</td>" +
            '<td><span class="rr-meter"><i class="' + (vp >= 100 ? "full" : vp >= 75 ? "hot" : "") + '" style="width:' + vp + '%"></i></span></td>' +
          "</tr>";
        }).join("") + "</tbody></table></div>";
  ...
  function saveHunt(id) {
    ...
    var titles = val("#rrfTitles");
    if (!titles) { alert("Add at least one job title, so Role Recruiter knows who counts as a fit."); return; }
    var payload = {
      id: id || undefined,
      label: val("#rrfLabel") || titles.split(",")[0].trim(),
      titles: titles,
      credentials: val("#rrfCreds"),
      location: val("#rrfLocation"),
      locationId: val("#rrfLocId"),
      radiusMiles: Number(val("#rrfRadius")) || undefined,
      minYears: Number(val("#rrfYears")) || undefined,
      pitchRole: val("#rrfPitch"),
      keywords: val("#rrfKeywords") || titles.split(",").map(function (s) { return '"' + s.trim() + '"'; }).join(" OR ")
    };
    editing = null;
    run(function () { return act("hunt_save", payload); });
  }

  window.__RoleRecruiter = { render: render, refresh: refresh };
})();
