...
Reaching finance leaders by commenting on their posts instead of messaging them: on their hiring
post when they are hiring, and on the industry posts they write when they are not.

Status: **shipped 2026-08-15**, and it is now the desk's primary outbound lane, not a fallback for
closed profiles. Owner decision that day, after the DM lane was measured: 0 of 26 sampled posters
were reachable for free (no Open Profiles, no 1st-degree), so private messaging could not carry
8 to 10 touches a day at any price that made sense. Public comments can, and they cost nothing.

What changed on the way in:

- **The desk is CFO / finance.** The keyword bank is finance leadership titles, so every scenario,
  hiring or not, stays inside one market. Scattershot markets were open work item 2 below; a single
  vertical is what closes it.
- **Industry posts count.** A fourth default scenario, `industry_conversation`, finds finance
  leaders writing about the work itself (close, forecasting, cash flow, audit season). No hiring
  intent is required and the drafting prompt is told explicitly not to mention hiring, recruiting,
  or candidates on those posts. This is also what keeps the activity feed from being 100% cold
  hiring-post replies, which was open work item 3.
- **8 to 10 a day, every day** (owner spec). See the throttle table below for how that is enforced.
...
|---|---|---|
| Per day | 9 base | Jittered **plus or minus 15%** (so 8 to 10), seeded on workspace + date |
| Per week | 63 | Hard rolling 7-day ceiling, no jitter |
| Spacing | 24 to 95 min | Randomized gap after each comment |

The week must clear seven days of the day allowance or it, not the day, is the real limit: the
original pairing of 8/day with 35/week could only ever sustain 5/day, whatever the daily number
said. The jitter narrowed from 40% to 15% to sit inside the owner's 8-to-10 band; it is still there,
because a desk posting the same round number every single day is itself a pattern, but it now draws
from three values rather than seven.
...
read what actually goes out, and decide from there. That is what shipping on 2026-08-15 starts: the
first days are for reading comments, not for scaling them.
...
## Live settings to know

- **The lane defaults to enabled** (`commentLimitsFor` returns `enabled: true` when unset), and this
  workspace's autopilot is on, so comments post publicly without an approval step. That was the
  owner's call on shipping day; the off switch is the Comments tile on the Role Hunter card.
- **Comments cannot be deleted through the client.** `unipile.ts` has create, list, and react, but
  no delete, and the comment id that comes back is never persisted onto the item. Anything posted is
  posted, so the copy rules and the duplicate guard are the only real safety net.
- Existing `interactions` caps in `os/policy.ts` apply on top: `comment_post` counts as an
  interaction, and the daily target of 10 is shared with likes and endorsements. `weeklyTarget` is
