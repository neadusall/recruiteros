  ...
  function connectedCatalog() {
    return [
      ...
      { id: "jd_sourcing", label: "JD Sourcing (RapidAPI people search)", status: "red", requiredFor: [],
        ...
        steps: ["Subscribe to the Fresh LinkedIn Scraper API (by SaleLeads) on RapidAPI — the free plan is enough to start.", "SEARCH → host: fresh-linkedin-scraper-api.p.rapidapi.com · path: /api/v1/search/people?name={query}&page={page}&limit=10 · method: GET", "Paste your RapidAPI key, fill the three Search values above exactly, Save, then Test — it should go green.", "DEEP-VET is optional — leave the Deep-vet fields blank for now; candidate search works without them."],
